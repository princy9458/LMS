...
import { Briefcase, Plus, Loader2, Edit, Trash2, Search, AlertCircle, MapPin, Building, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function AdminJobsPage() {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/lms/jobs/${deleteId}`, {
        method: 'DELETE',
      });
      if (res.ok) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900 tracking-tight flex items-center gap-2">
            <Briefcase className="w-6 h-6 text-indigo-500" /> Job Board
          </h1>
          <p className="text-zinc-500 text-sm mt-1">Manage career opportunities and recruitment listings.</p>
        </div>
        <Link 
            ...
            <tbody className="divide-y divide-zinc-100 bg-white">
              {loading ? (
                ...
                paginatedJobs.map((job) => (
                  <tr key={job._id} className="hover:bg-zinc-50/50 transition duration-150">
                    <td className="px-6 py-4">
                      <div className="text-sm font-semibold text-zinc-900">{job.title}</div>
                      <div className="text-xs text-zinc-500 mt-0.5">{job.category || 'Professional'}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <Building className="w-4 h-4 text-zinc-400" />
                        <span className="text-sm text-zinc-700 font-medium">{job.employer?.companyName || 'Unknown Corp'}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 hidden sm:table-cell">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium border ${
                        job.type === 'Full-time' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
                        job.type === 'Contract' ? 'bg-blue-50 text-blue-700 border-blue-200' :
        ...
        {totalPages > 1 && (
          <div className="px-6 py-4 border-t border-zinc-200 bg-zinc-50/50 flex items-center justify-between">
            <div className="text-xs text-zinc-500 font-medium">
              Showing <span className="text-zinc-900">{(currentPage - 1) * itemsPerPage + 1}</span> to <span className="text-zinc-900">{Math.min(currentPage * itemsPerPage, filteredJobs.length)}</span> of <span className="text-zinc-900">{filteredJobs.length}</span> listings
            </div>
            <div className="flex items-center gap-2">
              <button 
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl p-6 w-full max-sm shadow-xl border border-zinc-200">
            <div className="flex items-center justify-center w-12 h-12 rounded-full bg-red-100 text-red-600 mb-4 mx-auto">
              <AlertCircle className="w-6 h-6" />
            </div>
            <h3 className="text-lg font-bold text-center text-zinc-900 mb-2">Delete Job</h3>
            <p className="text-center text-zinc-500 text-sm mb-6">
              Are you sure you want to delete this job listing? This will also remove any active applications.
            </p>
